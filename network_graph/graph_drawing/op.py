...
SUPPORTED_OPERATIONS = ['Add', 'Mul', 'Concat', 'Split', 'BatchNorm', 'Conv', 'Linear', 'Relu', 'Sigmoid', 'GlobalAveragePool', 'AveragePool', 'MaxPool']
...
class Operation():
    ...
    def draw_square(self, color = None):
        self.goto(self.x - int(self.op_size/2), self.y - int(self.op_size/2))
        if color is not None:
            t.fillcolor(color)
            t.begin_fill()
        for _ in range(4):
            t.forward(int(self.op_size)) # Forward turtle by op_size units
            t.left(90) # Turn turtle by 90 degree
        if color is not None: t.end_fill()
        self.goto(self.x + int(self.op_size/2), self.y)

    def draw_circle(self, color = None):
        # self.goto(self.x, self.y)
        self.goto(self.x, self.y - int(self.op_size/2))
        if color is not None:
            t.fillcolor(color)
            t.begin_fill()
        t.circle(int(self.op_size/2))
        if color is not None: t.end_fill()
        self.goto(self.x + int(self.op_size/2), self.y)
...
class Conv(Operation):
    def __init__(self, op_size, x, y, params):
        ...
        if 1 in kernel_shape:   self.draw_square(color = '#FAF0B3')
        elif 3 in kernel_shape: self.draw_square(color = '#F1D526')
        elif 5 in kernel_shape: self.draw_square(color = '#D6B900')
        elif 7 in kernel_shape: self.draw_square(color = '#A79000')
        elif 9 in kernel_shape: self.draw_square(color = '#776700')
        else: self.draw_square(color = '#473E00')
        self.goto(self.x, self.y)
        t.dot(int(self.op_size/8))
        self.goto(self.x + int(self.op_size/2), self.y)
...
class Add(Operation):
    def __init__(self, op_size, x, y, params):
        ...
        self.goto(x, y+int(self.op_size/2))
        t.setheading(270)
        t.forward(self.op_size)
        self.goto(self.x + int(self.op_size/2), self.y)
...
class Concat(Operation):
    def __init__(self, op_size, x, y, params):
        super().__init__(op_size, x, y, params)
        self.draw_square()
        self.goto(x-int(self.op_size/2), y+int(self.op_size/2))
        t.setheading(270+45) # south east
        t.forward(self.diag)
        self.goto(x-int(self.op_size/2), y-int(self.op_size/2))
