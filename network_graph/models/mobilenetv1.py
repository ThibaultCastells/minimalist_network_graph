...
class MobileNetV1(nn.Module):
    def __init__(self, compress_rate, num_classes=1000):
        ...
        def conv_dw(inp, oup, stride, inp_cprate, oup_cprate):
            return nn.Sequential(
                nn.Conv2d(int(inp * (1 - inp_cprate)), int(inp * (1 - inp_cprate)), 3, stride, 1,
                          groups=int(inp * (1 - inp_cprate)), bias=False),
...
def mobilenet_v1(compress_rate=[0.] * 100, num_classes=1000):
    return MobileNetV1(compress_rate=compress_rate, num_classes=num_classes)
